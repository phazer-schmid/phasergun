...
const embedding_service_1 = require("./src/embedding-service");
async function testEmbeddingService() {
    ...
    try {
        ...
        const embeddingService = embedding_service_1.EmbeddingService.getInstance(process.cwd());
        ...
        for (let i = 0; i < texts.length; i++) {
            const similarity = embedding_service_1.EmbeddingService.cosineSimilarity(queryEmbedding, embeddings[i]);
            console.log(`   - "${texts[i]}": ${(similarity * 100).toFixed(2)}%`);
        }
        console.log();
        // Test top-K retrieval
        console.log('5. Testing top-K retrieval...');
        const topK = embedding_service_1.EmbeddingService.findTopK(queryEmbedding, embeddings, 2);
