...
/**
 * POST /api/generate
 * Generate DHF document content from a prompt using RAG context
 */
app.post('/api/generate', async (req: Request, res: Response) => {
  const { projectPath, promptFilePath } = req.body;
  ...
  if (!promptFilePath) {
    return res.status(400).json({
      status: 'error',
      message: 'Prompt file path is required',
      timestamp: new Date().toISOString()
    });
  }
  ...
  console.log(`[API] Project: ${projectPath}`);
  console.log(`[API] Prompt: ${path.basename(promptFilePath)}`);
  console.log(`[API] ========================================\n`);

  try {
    // Read prompt file
    const promptContent = await fs.readFile(promptFilePath, 'utf-8');
    const promptName = path.basename(promptFilePath, path.extname(promptFilePath));
    
    console.log(`[API] Prompt loaded: ${promptName}`);
    console.log(`[API] Prompt length: ${promptContent.length} characters\n`);

    // Load RAG knowledge using enhanced RAG service
    const { EnhancedRAGService } = await import('../../rag-service/src/enhanced-rag-service.js');
    ...
    console.log(`[API]   - Primary Context: ${metadata.primaryContextLoaded ? '✓' : '✗'}`);
    console.log(`[API]   - Procedures: ${metadata.proceduresCount} files`);
    console.log(`[API]   - Context Files: ${metadata.contextFilesCount} files`);
    console.log(`[API]   - Cache Fingerprint: ${metadata.fingerprint}\n`);
...
Generate the requested content based on the prompt above. Use the knowledge from:
1. PRIMARY CONTEXT - Your role as PhaserGun AI Regulatory Engineer
2. COMPANY PROCEDURES AND SOPS - Follow these guidelines strictly
3. PROJECT-SPECIFIC CONTEXT - Use this information about the current project

Generate high-quality, compliant DHF document content that adheres to all provided guidelines and context.`;
    ...
    // Generate content with selected LLM service
    if (llmService) {
      console.log('[API] Calling LLM API for content generation...');
      const llmResponse = await llmService.generateText(fullPrompt);
      ...
      generatedContent = `📄 Generated DHF Content (MOCK MODE)

Prompt: ${promptName}

This is MOCK generated content. Set LLM_MODE and API key to get real AI-generated content.

RAG Context Used:
✅ Primary Context: PhaserGun AI Regulatory Engineer role loaded
✅ Procedures: ${metadata.proceduresCount} company SOP files
✅ Context: ${metadata.contextFilesCount} project-specific files

To enable REAL content generation:
1. Set ANTHROPIC_API_KEY (or other LLM provider key) in .env
2. Set LLM_MODE=anthropic (or mistral/groq/ollama)
...
Generated at: ${new Date().toLocaleTimeString()}`;
    ...
    res.json({
      ...
      metadata: {
        ...
        llmMode,
        ragSources: {
          primaryContext: metadata.primaryContextLoaded,
          proceduresCount: metadata.proceduresCount,
          contextFilesCount: metadata.contextFilesCount,
          cacheFingerprint: metadata.fingerprint
        }
      }
    });

  } catch (error) {
    console.error('\n[API] ❌ Generation error:', error);
    res.status(500).json({
      status: 'error',
